...
def reverse_forward_tunnel(server_port, remote_host, remote_port, transport):
    # transport负责建立和维持加密通信
    # 将服务器上某个端口的TCP连接全部转发过来
    transport.request_port_forward("", server_port)
    while True:
        # channel主要用于加密的transport中收发数据
        # 建立一个新的channel
...
# python3 rforward.py [ssh-server-ip] -p [打开的port] -r [forwarding-ip]:[port] --user=[usr] --password
# 连接SSH服务器后, 打开某端口, 该端口将流量转发给forwarding ip的端口
# SSH隧道发给服务器一串用SSH加密过的网络流量, 服务器收到后将其解密, 并转发给真正的接收者